...
function setDelay(difficulty) {
  if(difficulty==="easy") {
    ...
    throw("Difficulty options are easy, normal, or hard");
  }
}
...
function chooseHole(holes) {
  ...
  const nextHole = holes[randomInt];
  if(randomInt===lastHole) {
    chooseHole(holes);
  }
  lastHole=randomInt;
  return holes[randomInt];
}
...
function toggleVisibility(hole){
 ...
 // event.target.classList.add('mole-hit');
  mole = hole.children[0];
